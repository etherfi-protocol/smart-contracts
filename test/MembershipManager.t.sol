...
contract MembershipManagerTest is TestSetup {
    ...
    function test_withdrawalPenalty() public {
        ...
        vm.prank(bob);
        uint256 bobCheckpointId = withdrawRequestNFTInstance.findCheckpointIndex(bobTokenId, 0, 2);
        withdrawRequestNFTInstance.claimWithdraw(bobTokenId, bobCheckpointId);
    ...
    function test_batchClaimWithdraw() public {
        ...
        // TODO - create batch claim function
    ...
    function test_HowPointsGrow() public {
        ...
        vm.startPrank(alice);
        // withdrawRequestNFTInstance.(aliceRequestId1);
        assertEq(membershipNftInstance.loyaltyPointsOf(tokenId), 2 * kwei);
        assertEq(membershipNftInstance.tierPointsOf(tokenId), 0);
        ...
        vm.startPrank(alice);
        uint256 aliceCheckpointId = withdrawRequestNFTInstance.findCheckpointIndex(aliceRequestId2, 0, 2);
        withdrawRequestNFTInstance.claimWithdraw(aliceRequestId2, aliceCheckpointId);
    ...
    function test_requestWithdraw() public {
        ...
        vm.startPrank(alice);
        uint256 aliceCheckpointId = withdrawRequestNFTInstance.findCheckpointIndex(requestId, 0, 2);
        withdrawRequestNFTInstance.claimWithdraw(requestId, aliceCheckpointId);
        assertEq(eETHInstance.balanceOf(alice), 0 ether);
        assertEq(membershipNftInstance.valueOf(aliceToken), 1 ether);
        assertEq(alice.balance, 1 ether);
