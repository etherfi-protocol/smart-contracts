...
contract TimelockTest is TestSetup {
    ...
    */
    ...
    function test_rollback_upgrade() public {
        ...
        ///////////////////// Upgrade ////////////////////
        address[] memory _targetsUpgrade = new address[](17);
        bytes[] memory _dataUpgrade = new bytes[](17);
        uint256[] memory _valuesUpgrade = new uint256[](17);

        /////////////////////////////////////////////////////////

        ///////////////////// Rollback Vars ////////////////////
        address[] memory _targetsRollback = new address[](8);
        bytes[] memory _dataRollback = new bytes[](8);
        uint256[] memory _valuesRollback = new uint256[](8);

        for (uint256 i = 0; i < _valuesRollback.length; i++) {
            _valuesRollback[i] = 0;
        }
        //////////////////////////////  ////////////////////////
        ...
        /////////////////////////// Values pre Upgrade //////////////////////
        uint256 blockBeforeUpgrade = 22977373;
        initializeRealisticForkWithBlock(MAINNET_FORK, blockBeforeUpgrade);

        for (uint256 i = 0; i < 9; i++) {
            _targetsUpgrade[i] = address(roleRegistryInstance);
        }

        address EtherFiNodeBeacon = address(0x3c55986Cfee455E2533F4D29006634EcF9B7c03F);
        UpgradeableBeacon EtherFiNodeBeaconProxy = UpgradeableBeacon(EtherFiNodeBeacon);
        console2.log(address(roleRegistryInstance));

        //1. Get implementations of the contracts we need to upgrade
        address EETHImplBefore = addressProviderInstance.getImplementationAddress("EETH");
        address EtherFiAdminImplBefore = addressProviderInstance.getImplementationAddress("EtherFiAdmin");
        address EtherFiNodeImplBefore = EtherFiNodeBeaconProxy.implementation();
        address EtherFiNodesManagerImplBefore = addressProviderInstance.getImplementationAddress("EtherFiNodesManager");
        address EtherFiOracleImplBefore = addressProviderInstance.getImplementationAddress("EtherFiOracle");
        address LiquidityPoolImplBefore = addressProviderInstance.getImplementationAddress("LiquidityPool");
        address StakingManagerImplBefore = addressProviderInstance.getImplementationAddress("StakingManager");
        address WeETHImplBefore = addressProviderInstance.getImplementationAddress("WeETH");

        ////////////////////////////////////////////////////////////////////////////////////


        ///////////////////// Change to Implementation instead of Constructing ////////////////////
        ...
        ///////////////////// Upgrade ////////////////////
        address staker1 = address(0x2aCA71020De61bb532008049e1Bd41E451aE8AdC);
        //role registry txs
        _dataUpgrade[0] = abi.encodeWithSelector(RoleRegistry.grantRole.selector, etherFiNodeImpl.ETHERFI_NODE_EIGENLAYER_ADMIN_ROLE(), address(managerInstance));
        _dataUpgrade[1] = abi.encodeWithSelector(RoleRegistry.grantRole.selector, etherFiNodeImpl.ETHERFI_NODE_EIGENLAYER_ADMIN_ROLE(), address(stakingManagerInstance));
        _dataUpgrade[2] = abi.encodeWithSelector(RoleRegistry.grantRole.selector, etherFiNodeImpl.ETHERFI_NODE_EIGENLAYER_ADMIN_ROLE(), address(staker1));

        _dataUpgrade[3] = abi.encodeWithSelector(RoleRegistry.grantRole.selector, etherFiNodeImpl.ETHERFI_NODE_CALL_FORWARDER_ROLE(), staker1); //fixxxxx

        _dataUpgrade[3] = abi.encodeWithSelector(RoleRegistry.grantRole.selector, etherFiNodesManagerImpl.ETHERFI_NODES_MANAGER_ADMIN_ROLE(), staker1);
        _dataUpgrade[4] = abi.encodeWithSelector(RoleRegistry.grantRole.selector, etherFiNodesManagerImpl.ETHERFI_NODES_MANAGER_CALL_FORWARDER_ROLE(), staker1);
        _dataUpgrade[5] = abi.encodeWithSelector(RoleRegistry.grantRole.selector, etherFiNodesManagerImpl.ETHERFI_NODES_MANAGER_EIGENLAYER_ADMIN_ROLE(), staker1);

        _dataUpgrade[6] = abi.encodeWithSelector(RoleRegistry.grantRole.selector, stakingManagerImpl.STAKING_MANAGER_NODE_CREATOR_ROLE(), staker1);

        _dataUpgrade[7] = abi.encodeWithSelector(RoleRegistry.grantRole.selector, liquidityPoolImpl.LIQUIDITY_POOL_ADMIN_ROLE(), staker1);
        _dataUpgrade[8] = abi.encodeWithSelector(RoleRegistry.grantRole.selector, liquidityPoolImpl.LIQUIDITY_POOL_VALIDATOR_APPROVER_ROLE(), staker1);
        //upgrade
        _dataUpgrade[9] = abi.encodeWithSelector(UUPSUpgradeable.upgradeTo.selector, address(EETHImpl));
        _dataUpgrade[10] = abi.encodeWithSelector(UUPSUpgradeable.upgradeTo.selector, address(etherFiAdminImpl));
        _dataUpgrade[11] = abi.encodeWithSelector(StakingManager.upgradeEtherFiNode.selector, address(etherFiNodeImpl));
        _dataUpgrade[12] = abi.encodeWithSelector(UUPSUpgradeable.upgradeTo.selector, address(etherFiNodesManagerImpl));
        _dataUpgrade[13] = abi.encodeWithSelector(UUPSUpgradeable.upgradeTo.selector, address(etherFiOracleImpl));
        _dataUpgrade[14] = abi.encodeWithSelector(UUPSUpgradeable.upgradeTo.selector, address(liquidityPoolImpl));
        _dataUpgrade[15] = abi.encodeWithSelector(UUPSUpgradeable.upgradeTo.selector, address(stakingManagerImpl));
        _dataUpgrade[16] = abi.encodeWithSelector(UUPSUpgradeable.upgradeTo.selector, address(WeETHImpl));

        _targetsUpgrade[9] = address(eETHInstance);
        _targetsUpgrade[10] = address(etherFiAdminInstance);
        _targetsUpgrade[11] = address(stakingManagerInstance);
        _targetsUpgrade[12] = address(managerInstance);
        _targetsUpgrade[13] = address(etherFiOracleInstance);
        _targetsUpgrade[14] = address(liquidityPoolInstance);
        _targetsUpgrade[15] = address(stakingManagerInstance);
        _targetsUpgrade[16] = address(weEthInstance);

        //3. Create calls to rollback the upgrade the contracts
        _targetsRollback[1] = address(eETHInstance);
        _targetsRollback[0] = address(etherFiAdminInstance);
        _targetsRollback[2] = address(stakingManagerInstance);
        _targetsRollback[3] = address(managerInstance);
        _targetsRollback[4] = address(etherFiOracleInstance);
        _targetsRollback[5] = address(liquidityPoolInstance);
        _targetsRollback[6] = address(stakingManagerInstance);
        _targetsRollback[7] = address(weEthInstance);

        _dataRollback[1] = abi.encodeWithSelector(UUPSUpgradeable.upgradeTo.selector, EETHImplBefore);
        _dataRollback[0] = abi.encodeWithSelector(UUPSUpgradeable.upgradeTo.selector, EtherFiAdminImplBefore);
        _dataRollback[2] = abi.encodeWithSelector(UUPSUpgradeable.upgradeTo.selector, StakingManagerImplBefore);
        _dataRollback[3] = abi.encodeWithSelector(UUPSUpgradeable.upgradeTo.selector, EtherFiNodesManagerImplBefore);
        _dataRollback[4] = abi.encodeWithSelector(UUPSUpgradeable.upgradeTo.selector, EtherFiOracleImplBefore);
        _dataRollback[5] = abi.encodeWithSelector(UUPSUpgradeable.upgradeTo.selector, LiquidityPoolImplBefore);
        _dataRollback[6] = abi.encodeWithSelector(StakingManager.upgradeEtherFiNode.selector, address(EtherFiNodeImplBefore));
        _dataRollback[7] = abi.encodeWithSelector(UUPSUpgradeable.upgradeTo.selector, WeETHImplBefore);
