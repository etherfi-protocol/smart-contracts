...
contract PreludeTest is Test, ArrayTestHelper {
    ...
    NodeOperatorManager nodeOperatorManager =
        NodeOperatorManager(0xd5edf7730ABAd812247F6F54D7bd31a52554e35E);
    ...
    address etherFiNodeBeacon =
        address(0x3c55986Cfee455E2533F4D29006634EcF9B7c03F);
    RoleRegistry roleRegistry =
        RoleRegistry(0x62247D29B4B9BECf4BB73E0c722cf6445cfC7cE9);
    IStrategy beaconStrategy =
        IStrategy(address(0xbeaC0eeEeeeeEEeEeEEEEeeEEeEeeeEeeEEBEaC0));
    ...
    function setUp() public {
        ...
        roleRegistry.grantRole(
            etherFiNodeImpl.ETHERFI_NODE_EIGENLAYER_ADMIN_ROLE(),
            address(etherFiNodesManager)
        );
        roleRegistry.grantRole(
            etherFiNodeImpl.ETHERFI_NODE_EIGENLAYER_ADMIN_ROLE(),
        ...
        roleRegistry.grantRole(
            etherFiNodeImpl.ETHERFI_NODE_CALL_FORWARDER_ROLE(),
            address(etherFiNodesManager)
        );
        roleRegistry.grantRole(
        ...
        roleRegistry.grantRole(
            etherFiNodesManager.ETHERFI_NODES_MANAGER_EIGENLAYER_ADMIN_ROLE(),
            eigenlayerAdmin
        );
        roleRegistry.grantRole(
    ...
    function helper_createValidator(
        TestValidatorParams memory _params
    ) public returns (TestValidator memory) {
        // create a copy or else successive calls of this method can mutate the input unexpectedly
        TestValidatorParams memory params = TestValidatorParams(
            _params.nodeOperator,
            _params.etherFiNode,
            _params.bidId,
            _params.validatorSize,
            _params.withdrawable
        );
        ...
        if (params.nodeOperator == address(0)) {
            ...
            if (!nodeOperatorManager.registered(params.nodeOperator)) {
                ...
                nodeOperatorManager.registerNodeOperator(
                    "test_ipfs_hash",
                    1000
                );
            }
        ...
        if (params.bidId == 0) {
            ...
            vm.prank(params.nodeOperator);
            params.bidId = auctionManager.createBid{value: 0.1 ether}(
                1,
                0.1 ether
            )[0];
        }
        ...
        if (params.etherFiNode == address(0)) {
            vm.prank(admin);
            params.etherFiNode = stakingManager.instantiateEtherFiNode(
                /*createEigenPod=*/ true
            );
        }
        ...
        address eigenPod = address(
            IEtherFiNode(params.etherFiNode).getEigenPod()
        );
        bytes32 initialDepositRoot = depositDataRootGenerator
            .generateDepositDataRoot(
                pubkey,
                signature,
        ...
        IStakingManager.DepositData memory initialDepositData = IStakingManager
            .DepositData({
                publicKey: pubkey,
                signature: signature,
        ...
        stakingManager.createBeaconValidators{value: 1 ether}(
            toArray(initialDepositData),
            toArray_u256(params.bidId),
            params.etherFiNode
        );
        ...
        // remaining deposit
        bytes32 confirmDepositRoot = depositDataRootGenerator
            .generateDepositDataRoot(
                pubkey,
                signature,
                etherFiNodesManager.addressToCompoundingWithdrawalCredentials(
        ...
        IStakingManager.DepositData memory confirmDepositData = IStakingManager
            .DepositData({
                publicKey: pubkey,
                signature: signature,
        ...
        vm.prank(address(liquidityPool));
        stakingManager.confirmAndFundBeaconValidators{value: confirmAmount}(
            toArray(confirmDepositData),
            params.validatorSize
        );
        ...
        if (params.withdrawable) {
            ...
            address eigenpod = etherFiNodesManager.getEigenPod(
                uint256(params.bidId)
            );
            vm.store(
                eigenpod,
                bytes32(uint256(52)) /*slot*/,
            ...
            IEigenPodManager(eigenPodManager).addShares(
                params.etherFiNode,
                beaconStrategy,
                params.validatorSize
            );
    ...
    function test_validatorHelper() public {
        ...
        TestValidator memory val = helper_createValidator(
            defaultTestValidatorParams
        );
        console2.log("------------------------------------");
        ...
        TestValidator memory val2 = helper_createValidator(
            defaultTestValidatorParams
        );
        console2.log("------------------------------------");
        ...
        assertGe(
            IEigenPod(val4.eigenPod).withdrawableRestakedExecutionLayerGwei(),
            (2000 ether / 1 gwei)
        );
    ...
    function test_forwardingWhitelist() public {
        // create a node + pod
        vm.prank(admin);
        IEtherFiNode etherFiNode = IEtherFiNode(
            stakingManager.instantiateEtherFiNode(true /*createEigenPod*/)
        );

        // link it to an arbitrary id
        uint256 legacyID = 10885;
        bytes
            memory pubkey = hex"8f9c0aab19ee7586d3d470f132842396af606947a0589382483308fdffdaf544078c3be24210677a9c471ce70b3b4c2c";
        vm.prank(admin);
        etherFiNodesManager.linkLegacyValidatorIds(
            toArray_u256(legacyID),
        ...
        vm.startPrank(user);
        {
            ...
            etherFiNodesManager.forwardEigenPodCall(
                toArray_u256(legacyID),
                toArray_bytes("")
            );

            vm.expectRevert(IEtherFiNodesManager.IncorrectRole.selector);
            etherFiNodesManager.forwardExternalCall(
                toArray_u256(legacyID),
                toArray_bytes(""),
                address(0)
            );
        ...
        vm.startPrank(roleRegistry.owner());
        {
            ...
            roleRegistry.grantRole(
                EtherFiNode(payable(address(etherFiNode)))
                    .ETHERFI_NODE_CALL_FORWARDER_ROLE(),
                user
            );
        }
        vm.stopPrank();
        ...
        vm.startPrank(user);
        {
            ...
            vm.expectRevert(
                IEtherFiNodesManager.ForwardedCallNotAllowed.selector
            );
            etherFiNodesManager.forwardEigenPodCall(
                toArray_u256(legacyID),
                toArray_bytes(checkpointData)
            );
        ...
        vm.startPrank(admin);
        {
            ...
            etherFiNodesManager.updateAllowedForwardedEigenpodCalls(
                checkpointSelector,
                true
            );
        }
    ...
    function test_createBeaconValidators() public {
        ...
        bytes
            memory pubkey = hex"8f9c0aab19ee7586d3d470f132842396af606947a0589382483308fdffdaf544078c3be24210677a9c471ce70b3b4c2c";
        bytes
            memory signature = hex"877bee8d83cac8bf46c89ce50215da0b5e370d282bb6c8599aabdbc780c33833687df5e1f5b5c2de8a6cd20b6572c8b0130b1744310a998e1079e3286ff03e18e4f94de8cdebecf3aaac3277b742adb8b0eea074e619c20d13a1dda6cba6e3df";
    ...
    function test_withdrawRestakedValidatorETH() public {
        bytes
            memory validatorPubkey = hex"892c95f4e93ab042ee39397bff22cc43298ff4b2d6d6dec3f28b8b8ebcb5c65ab5e6fc29301c1faee473ec095f9e4306";
        bytes32 pubkeyHash = etherFiNodesManager.calculateValidatorPubkeyHash(
            validatorPubkey
        );
        uint256 legacyID = 10885;
    ...
    function test_EtherFiNodePermissions() public {
        ...
        vm.prank(admin);
        IEtherFiNode etherFiNode = IEtherFiNode(
            stakingManager.instantiateEtherFiNode(true)
        );
        ...
        BeaconChainProofs.BalanceProof[]
            memory balanceProofs = new BeaconChainProofs.BalanceProof[](1);
        BeaconChainProofs.BalanceContainerProof
            memory containerProof = BeaconChainProofs.BalanceContainerProof({
        ...
        IDelegationManager.QueuedWithdrawalParams[]
            memory params = new IDelegationManager.QueuedWithdrawalParams[](1);
        vm.expectRevert(IEtherFiNode.IncorrectRole.selector);
        etherFiNode.queueWithdrawals(params);

        IDelegationManager.Withdrawal[]
            memory withdrawals = new IDelegationManager.Withdrawal[](1);
    ...
    function test_EtherFiNodesManagerPermissions() public {
        ...
        vm.expectRevert(IEtherFiNodesManager.IncorrectRole.selector);
        etherFiNodesManager.verifyCheckpointProofs(
            nodeId,
            containerProof,
            balanceProofs
        );
        ...
        bool[] memory receiveAsTokens = new bool[](1);
        vm.expectRevert(IEtherFiNodesManager.IncorrectRole.selector);
        etherFiNodesManager.completeQueuedWithdrawals(
            nodeId,
            withdrawals,
            tokens,
            receiveAsTokens
    ...
    function test_StakingManagerPermissions() public {
        ...
        bytes32 initialDepositDataRoot = depositDataRootGenerator
            .generateDepositDataRoot(pubkey, signature, "", 1 ether);
        IStakingManager.DepositData memory initialDepositData = IStakingManager
            .DepositData({
                publicKey: pubkey,
                signature: signature,
        ...
        bytes32 confirmDepositDataRoot = depositDataRootGenerator
            .generateDepositDataRoot(pubkey, signature, "", 31 ether);
        IStakingManager.DepositData memory confirmDepositData = IStakingManager
            .DepositData({
                publicKey: pubkey,
                signature: signature,
    ...
    function test_getEigenPod() public {
        ...
        IEtherFiNode node = IEtherFiNode(
            0xbD0BFF833DE891aDcFF6Ee5502B23f516bECBf6F
        );
        ...
        IEtherFiNode newNode = IEtherFiNode(
            stakingManager.instantiateEtherFiNode(/*createEigenPod=*/ false)
        );
    ...
    function test_pubkeyHashAndLegacyId() public {
        ...
        address a2 = etherFiNodesManager.etherfiNodeAddress(
    ...
    function test_withdrawMultipleLargeValidators() public {
        ...
        vm.prank(eigenlayerAdmin);
        etherFiNodesManager.queueETHWithdrawal(
        ...
        assertEq(
            address(liquidityPool).balance,
            startingLPBalance + 1234 ether
        );
    }
