...
contract EtherFiRestakerTest is TestSetup {
    ...
    function test_queueWithdrawals_2() public returns (bytes32[] memory) {
        ...
        params[0] = IDelegationManagerTypes.QueuedWithdrawalParams({strategies: strategies, depositShares: withdrawableShares, __deprecated_withdrawer: address(etherFiRestakerInstance)});
    ...
    function test_delegate_to() public {
        ...
        ISignatureUtilsMixinTypes.SignatureWithExpiry memory signature = ISignatureUtilsMixinTypes.SignatureWithExpiry({signature: hex"", expiry: 0});
