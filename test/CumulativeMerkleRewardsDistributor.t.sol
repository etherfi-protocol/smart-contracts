...
contract CumulativeMerkleRewardsDistributorTest is TestSetup {
    ...
    function setMerkleRoot(address token) internal {
        ...
        vm.warp(block.timestamp + 15_000 * 12);
        cumulativeMerkleRewardsDistributorInstance.finalizeMerkleRoot(token, block.number - 15_000);
        vm.stopPrank();
    }
    ...
    function test_claiming_delay() public {
        ...
        vm.warp(block.timestamp + 14_399 * 12);
        vm.expectRevert(ICumulativeMerkleRewardsDistributor.InsufficentDelay.selector);
        cumulativeMerkleRewardsDistributorInstance.finalizeMerkleRoot(address(eETHInstance), block.number - 12_000);
