...
contract EtherFiAdmin is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function _handleWithdrawals(IEtherFiOracle.OracleReport calldata _report) internal {
        ...
        // Only necessary to finalize request if new request have been finalized since the last oracle report
        if (withdrawRequestNft.lastFinalizedRequestId() < _report.lastFinalizedWithdrawalRequestId) {
