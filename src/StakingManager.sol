...
contract StakingManager is Initializable, IStakingManager, IBeaconUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function createBeaconValidators(DepositData[] calldata depositData, uint256[] calldata bidIds, address etherFiNode) external payable {
        ...
        if (depositData.length != bidIds.length) {
            revert InvalidDepositData();
        }
        if (address(IEtherFiNode(etherFiNode).getEigenPod()) == address(0)) {
        ...
        for (uint256 i = 0; i < depositData.length; i++) {
            // claim the bid
            if (!auctionManager.isBidActive(bidIds[i])) {
                revert InactiveBid();
            }
    ...
    function confirmAndFundBeaconValidators(DepositData[] calldata depositData, uint256 validatorSizeWei) external payable {
        ...
        for (uint256 i = 0; i < depositData.length; i++) {
            ...
            if (address(etherFiNode) == address(0x0)) {
                revert UnlinkedPubkey();
            }
    ...
    function instantiateEtherFiNode(bool _createEigenPod) external returns (address) {
        if (!roleRegistry.hasRole(STAKING_MANAGER_NODE_CREATOR_ROLE, msg.sender)) {
            revert IncorrectRole();
        }
    ...
    modifier onlyAdmin() {
        if (!roleRegistry.hasRole(STAKING_MANAGER_ADMIN_ROLE, msg.sender)) {
            revert IncorrectRole();
        }
        _;
    }
