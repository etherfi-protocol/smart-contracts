...
contract PermissionMulticall {
    ...
    modifier onlyEtherfi() {
        require(whitelist[msg.sender], "Only Etherfi function");
        _;
    }
    ...
    function updateWhitelist(address _user, bool _allowed) external onlyOwner {
    ...
    function tryAggregate(
        bool requireSuccess,
        Call[] calldata calls
    ) public payable onlyEtherfi returns (Result[] memory returnData) {
        uint256 length = calls.length;
