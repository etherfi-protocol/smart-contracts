...
contract EtherFiNode is IEtherFiNode {
    ...
    function callEigenPod(bytes calldata data) external onlyEtherFiNodeManagerContract {
        (bool success, bytes memory reason) = address(eigenPod).call{gas: gasleft()}(data);
        if (!success) revertFromReturnedData(reason);
    }
    ...
    function forwardCall(address to, bytes memory data) external onlyEtherFiNodeManagerContract {
        (bool success, bytes memory reason) = address(to).call{gas: gasleft()}(data);
        if (!success) revertFromReturnedData(reason);
    }
