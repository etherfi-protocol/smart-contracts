...
contract Pauser {
    ...
    constructor(IPausable[] memory _pausables) {
     ...
     */
    function addPausable(IPausable _pausable) external {
     ...
     * @dev Callable by PAUSER_ADMIN_ROLE.
     */
    function removePausable(uint256 index) external {
     ...
     * @dev Callable by PAUSER_ADMIN_ROLE.
     */
    function updateSenderToPausable(address sender, IPausable pausable) external {
     ...
     */
    function pauseSingle(IPausable pausable) external {
        pausable.pause();
     ...
     */
    function unpauseSingle(IPausable pausable) external {
        pausable.unpause();
     ...
     * @dev Callable by GENERIC_PAUSER_ROLE.
     */
    function pauseMultiple(IPausable[] calldata _pausables) external {
        for (uint256 i = 0; i < _pausables.length; ++i) {
            _pausables[i].pause();
     ...
     * @dev Callable by GENERIC_UNPAUSER_ROLE.
     */
    function unpauseMultiple(IPausable[] calldata _pausables) external {
        for (uint256 i = 0; i < _pausables.length; ++i) {
     ...
     */
    function senderUnpause() external {
        IPausable pausable = senderToPausable[msg.sender];
        pausable.unpause();
