...
contract NodeOperatorManager is INodeOperatorManager, Initializable, UUPSUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function initializeOnUpgrade(address[] memory _operator, bytes[] memory _ipfsHash, uint64[] memory _totalKeys, uint64[] memory _keysUsed) external onlyOwner {
        ...
        for (uint256 x = 0; x < _operator.length; x++) {
            ...
            KeyData memory keyData = KeyData({totalKeys: _totalKeys[x], keysUsed: _keysUsed[x], ipfsHash: abi.encodePacked(_ipfsHash[x])});

            addressToOperatorData[_operator[x]] = keyData;
            registered[_operator[x]] = true;

            emit OperatorRegistered(_operator[x], keyData.totalKeys, keyData.keysUsed, _ipfsHash[x]);
        }
    }
    ...
    function registerNodeOperator(bytes memory _ipfsHash, uint64 _totalKeys) public whenNotPaused {
        require(!registered[msg.sender], "Already registered");

        KeyData memory keyData = KeyData({totalKeys: _totalKeys, keysUsed: 0, ipfsHash: abi.encodePacked(_ipfsHash)});

        addressToOperatorData[msg.sender] = keyData;
        registered[msg.sender] = true;

        emit OperatorRegistered(msg.sender, keyData.totalKeys, keyData.keysUsed, _ipfsHash);
    }
    ...
    function fetchNextKeyIndex(address _user) external onlyAuctionManagerContract returns (uint64) {
        ...
        require(keyData.keysUsed < totalKeys, "Insufficient public keys");
    ...
    function batchUpdateOperatorsApprovedTags(address[] memory _users, LiquidityPool.SourceOfFunds[] memory _approvedTags, bool[] memory _approvals) external onlyAdmin {
    ...
    function getNumKeysRemaining(address _user) external view returns (uint64 numKeysRemaining) {
        ...
        numKeysRemaining = keyData.totalKeys - keyData.keysUsed;
    }
    ...
    modifier onlyAuctionManagerContract() {
        require(msg.sender == auctionManagerContractAddress, "Only auction manager contract function");
        _;
    }
