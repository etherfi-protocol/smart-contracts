...
contract DeployWeETHWithdrawAdapter is Script {
    ...
    function run() external {
        ...
        // Deploy WeETHWithdrawAdapter implementation
        {
            ...
            bytes memory constructorArgs = abi.encode(weETHAddress, eETHAddress, liquidityPoolAddress, withdrawRequestNFTAddress, roleRegistryAddress);
            bytes memory bytecode = abi.encodePacked(type(WeETHWithdrawAdapter).creationCode, constructorArgs);
        ...
        // Deploy UUPSProxy
        {
            ...
            bytes memory implConstructorArgs = abi.encode(weETHAddress, eETHAddress, liquidityPoolAddress, withdrawRequestNFTAddress, roleRegistryAddress);
            bytes memory implBytecode = abi.encodePacked(type(WeETHWithdrawAdapter).creationCode, implConstructorArgs);
            ...
            bytes memory initializerData = abi.encodeWithSelector(WeETHWithdrawAdapter.initialize.selector, timelockAddress);

            bytes memory constructorArgs = abi.encode(implementationAddress, initializerData);
    ...
    function computePredictedAddresses() internal {
        ...
        bytes memory initializerData = abi.encodeWithSelector(WeETHWithdrawAdapter.initialize.selector, timelockAddress);
        bytes memory proxyConstructorArgs = abi.encode(predictedImpl, initializerData);
        bytes memory proxyBytecode = abi.encodePacked(type(UUPSProxy).creationCode, proxyConstructorArgs);
    ...
    function deployContract(string memory contractName, bytes memory constructorArgs, bytes memory bytecode, bytes32 salt) internal returns (address) {
    ...
    function saveDeploymentLog(string memory contractName, address deployedAddress, bytes memory constructorArgs, bytes32 salt) internal {
