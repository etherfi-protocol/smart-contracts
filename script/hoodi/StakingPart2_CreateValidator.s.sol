...
contract StakingPart2 is Script {
    ...
    function run() external {
        ...
        bytes32 depositRoot = stakingManager.generateDepositDataRoot(pubkey, signature, withdrawalCredentials, 1 ether);

        console.log("!Ensure that this root matches the one in deposit_data.json! - Deposit root: %s", vm.toString(depositRoot));

        // Prepare deposit data
        IStakingManager.DepositData memory depositData = IStakingManager.DepositData({publicKey: pubkey, signature: signature, depositDataRoot: depositRoot, ipfsHashForEncryptedValidatorKey: "hoodi_testnet_validator"});
        ...
        vm.startBroadcast(vm.envUint("PRIVATE_KEY"));
        liquidityPool.batchRegister(depositDataArray, bidIdArray, etherFiNode);
        vm.stopBroadcast();
