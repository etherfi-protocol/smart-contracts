...
contract VerifyV3Upgrade is Script {
    // Mainnet addresses
    StakingManager stakingManager = StakingManager(0x25e821b7197B146F7713C3b89B6A4D83516B912d);
    ILiquidityPool liquidityPool = ILiquidityPool(0x308861A430be4cce5502d0A12724771Fc6DaF216);
    EtherFiNodesManager etherFiNodesManager = EtherFiNodesManager(payable(0x8B71140AD2e5d1E7018d2a7f8a288BD3CD38916F));
    AuctionManager auctionManager = AuctionManager(0x00C452aFFee3a17d9Cecc1Bcd2B8d5C7635C4CB9);
    ...
    function verifyRoleRegistryConfiguration() internal {
        ...
        checkCondition(address(stakingManager.roleRegistry()) == address(roleRegistry), "StakingManager has correct RoleRegistry");

        // Check EtherFiNodesManager has correct RoleRegistry
        console2.log("Checking EtherFiNodesManager RoleRegistry...");
        checkCondition(address(etherFiNodesManager.roleRegistry()) == address(roleRegistry), "EtherFiNodesManager has correct RoleRegistry");
        ...
        if (etherFiNodeImpl != address(0)) {
            ...
            checkCondition(address(nodeImpl.roleRegistry()) == address(roleRegistry), "EtherFiNode implementation has correct RoleRegistry");
        }

        // Check LiquidityPool has correct RoleRegistry
        console2.log("Checking LiquidityPool RoleRegistry...");
        checkCondition(address(IUpgrade(address(liquidityPool)).roleRegistry()) == address(roleRegistry), "LiquidityPool has correct RoleRegistry");

        // AuctionManager does not have RoleRegistry

        // Check eETH has correct RoleRegistry
        console2.log("Checking eETH RoleRegistry...");
        checkCondition(address(IUpgrade(eETH).roleRegistry()) == address(roleRegistry), "eETH has correct RoleRegistry");

        // Check weETH has correct RoleRegistry
        console2.log("Checking weETH RoleRegistry...");
        checkCondition(address(IUpgrade(weETH).roleRegistry()) == address(roleRegistry), "weETH has correct RoleRegistry");
        ...
        checkCondition(address(IUpgrade(etherFiAdmin).roleRegistry()) == address(roleRegistry), "EtherFiAdmin has correct RoleRegistry");
    }
    ...
    function verifyUpgradeability() internal {
        ...
        verifyProxyUpgradeability(address(etherFiNodesManager), "EtherFiNodesManager");
        ...
        checkCondition(protocolUpgrader != address(0), "Protocol upgrader (RoleRegistry owner) is set");
        ...
        bytes memory payload = abi.encodeWithSignature("upgradeTo(address)", protocolUpgrader); // any addr
        (bool success,) = address(stakingManager).call(payload);
        vm.stopPrank();

        checkCondition(!success, "Only protocol upgrader can execute upgradeTo()");
    ...
    function verifyProxyUpgradeability(address proxy, string memory name) internal {
        ...
        checkCondition(impl != address(0) && impl != proxy, string.concat(name, " is a proxy with an implementation"));

        // 2. Implementation exposes correct proxiableUUID()
        try IERC1822ProxiableUpgradeable(impl).proxiableUUID() returns (bytes32 slot) {
            checkCondition(slot == IMPLEMENTATION_SLOT, string.concat(name, " implementation returns correct UUID"));
        } catch {
            checkCondition(false, string.concat(name, " implementation missing proxiableUUID()"));
        }

        (bool ok, bytes memory data) = proxy.staticcall(abi.encodeWithSignature("upgradeTo(address)", impl));

        checkCondition(ok || data.length != 0, string.concat(name, " proxy exposes upgradeTo()"));
        vm.prank(address(0xcaffe));
        try IUpgrade(proxy).upgradeTo(address(0xbeef)) {
            checkCondition(false, string.concat(name, " allows a random to upgrade"));
        } catch {
            checkCondition(true, string.concat(name, " does not allows a random to upgrade"));
        }
    }
    ...
    function verifyRoleAssignments() internal {
        ...
        bytes32 STAKING_MANAGER_NODE_CREATOR_ROLE = keccak256("STAKING_MANAGER_NODE_CREATOR_ROLE");
        console2.log("Checking StakingManager roles...");

        // At least check that the role exists in the system
        checkCondition(STAKING_MANAGER_NODE_CREATOR_ROLE == stakingManager.STAKING_MANAGER_NODE_CREATOR_ROLE(), "STAKING_MANAGER_NODE_CREATOR_ROLE constant matches");

        // EtherFiNodesManager roles
        bytes32 ETHERFI_NODES_MANAGER_ADMIN_ROLE = keccak256("ETHERFI_NODES_MANAGER_ADMIN_ROLE");
        bytes32 ETHERFI_NODES_MANAGER_EIGENLAYER_ADMIN_ROLE = keccak256("ETHERFI_NODES_MANAGER_EIGENLAYER_ADMIN_ROLE");
        bytes32 ETHERFI_NODES_MANAGER_CALL_FORWARDER_ROLE = keccak256("ETHERFI_NODES_MANAGER_CALL_FORWARDER_ROLE");

        console2.log("Checking EtherFiNodesManager roles...");
        checkCondition(ETHERFI_NODES_MANAGER_ADMIN_ROLE == etherFiNodesManager.ETHERFI_NODES_MANAGER_ADMIN_ROLE(), "ETHERFI_NODES_MANAGER_ADMIN_ROLE constant matches");
        checkCondition(ETHERFI_NODES_MANAGER_EIGENLAYER_ADMIN_ROLE == etherFiNodesManager.ETHERFI_NODES_MANAGER_EIGENLAYER_ADMIN_ROLE(), "ETHERFI_NODES_MANAGER_EIGENLAYER_ADMIN_ROLE constant matches");
        checkCondition(ETHERFI_NODES_MANAGER_CALL_FORWARDER_ROLE == etherFiNodesManager.ETHERFI_NODES_MANAGER_CALL_FORWARDER_ROLE(), "ETHERFI_NODES_MANAGER_CALL_FORWARDER_ROLE constant matches");
        ...
        checkCondition(PROTOCOL_PAUSER == roleRegistry.PROTOCOL_PAUSER(), "PROTOCOL_PAUSER constant matches");
        checkCondition(PROTOCOL_UNPAUSER == roleRegistry.PROTOCOL_UNPAUSER(), "PROTOCOL_UNPAUSER constant matches");
    }
    ...
    function verifyContractInteractions() internal {
        ...
        checkCondition(address(stakingManager.liquidityPool()) == address(liquidityPool), "StakingManager has correct LiquidityPool reference");
        checkCondition(address(stakingManager.etherFiNodesManager()) == address(etherFiNodesManager), "StakingManager has correct NodesManager reference");
        checkCondition(address(stakingManager.auctionManager()) == address(auctionManager), "StakingManager has correct AuctionManager reference");

        // Verify EtherFiNodesManager knows about StakingManager
        console2.log("Checking EtherFiNodesManager contract references...");
        checkCondition(address(etherFiNodesManager.stakingManager()) == address(stakingManager), "EtherFiNodesManager has correct StakingManager reference");
    ...
    function verifyAdditionalContracts() internal {
        ...
        try IEtherFiOracle(etherFiOracle).consensusVersion() returns (uint32 version) {
            checkCondition(version > 0, "EtherFiOracle consensusVersion() is accessible");
        } catch {
        ...
        try IeETH(eETH).totalShares() returns (uint256 shares) {
            checkCondition(shares > 0, "eETH totalShares() returns valid value");
        } catch {
        ...
        try IWeETH(weETH).eETH() returns (IeETH eethContract) {
            checkCondition(address(eethContract) == eETH, "weETH correctly references eETH");
        } catch {
            checkCondition(false, "weETH eETH() reference check failed");
        }
    }

    function checkCondition(bool condition, string memory description) internal {
        totalChecks++;
        if (condition) {
            passedChecks++;
