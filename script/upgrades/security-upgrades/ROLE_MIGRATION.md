...
## 6. Post-upgrade operational follow-ups (NOT in this script)

These must be performed by ops AFTER all three batches land. They are NOT
generated by `transactions.s.sol` and have no `_preflight` gate — track them
in the broadcast runbook.

### 6.1 `EtherFiRedemptionManager.initializeTokenParameters` per token

`EtherFiRedemptionManager.tokenToRedemptionInfo[token]` defaults to zero for
EETH and WEETH after the upgrade. Until `initializeTokenParameters` is called
for each, `redeemEEth` / `redeemWeEth` revert (verified on Tenderly VNet during
PR #420 review — see F3). Function is `onlyAdmin = OPERATION_TIMELOCK_ROLE`,
so it rides a follow-up `OPERATING_TIMELOCK` batch (2-day delay).

Signature:
```solidity
function initializeTokenParameters(
    address[] memory _tokens,
    uint16[]  memory _exitFeeSplitToTreasuryInBps,
    uint16[]  memory _exitFeeInBps,
    uint16[]  memory _lowWatermarkInBpsOfTvl,
    uint256[] memory _bucketCapacity,
    uint256[] memory _bucketRefillRate
) external onlyAdmin;
```
...
Post-upgrade there is no protocol-wide TRANSFER bucket; per-address throttling
is opt-in via `setAddressRateLimits` on EETH and WEETH (`OPERATION_MULTISIG_ROLE`).
Until you call this for known large holders (CEXes, bridges, the LP itself,
strategy vaults), their transfers are unthrottled. Ship a Day-0 Safe tx that
pre-seeds buckets for the top ~20 addresses.

### 6.3 Stale holders of the new 9 roles

The script grants exactly one holder per role. Any pre-existing holder is
asserted away by `_appendGrantCalls`. After execution, query `roleHolders(role)`
for each of the 9 and revoke any address that shouldn't be there with a
follow-up Safe tx (tier roles → `UPGRADE_TIMELOCK`; others → `OPERATING_TIMELOCK`
or `RevokeAdmin`).
