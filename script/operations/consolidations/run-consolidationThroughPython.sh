...
# chmod +x script/operations/consolidations/run-consolidationThroughPython.sh
#
# run-consolidationThroughPython.sh - Automated validator consolidation workflow
#
...
#
# Usage:
#   ./script/operations/consolidations/run-consolidationThroughPython.sh \
...
BATCH_SIZE=50 # number of consolidations per transaction
...
# Generate transactions using Python script (faster, no forge compilation required)
GENERATE_ARGS=(
    --input "$CONSOLIDATION_DATA"
    --output-dir "$OUTPUT_DIR"
    --batch-size "$BATCH_SIZE"
)

python3 "$SCRIPT_DIR/generate_gnosis_txns.py" "${GENERATE_ARGS[@]}" 2>&1 | tee "$OUTPUT_DIR/generate_txns.log"
GENERATE_EXIT_CODE=${PIPESTATUS[0]}

if [ $GENERATE_EXIT_CODE -ne 0 ]; then
    echo -e "${RED}Error: Failed to generate transaction files${NC}"
    exit 1
fi
...
for generated_file in "$OUTPUT_DIR"/*.json; do
    ...
    # Skip consolidation-data.json and targets.json
    if [ "$filename" != "consolidation-data.json" ] && [ "$filename" != "targets.json" ]; then
